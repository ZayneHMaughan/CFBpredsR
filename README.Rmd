...
vignette: >
  %\VignetteIndexEntry{CFB_Analysis}
...
I also check certain variables that are highly correlated and removed some of those
...
bind_rows(
  ...
  get_metrics(lasso_submodel, smote_test,playoff, "LASSO" ),
...
Out of these three, I would say that the XGBoost performed the best. They
...
explainer <- explain(
  model = svm_sub_model,
  data = smote_test |> select(-playoff, -team),  # your test features
  y = smote_test$playoff,
  label = "SVM"
)
...
#### Conclusion
It seems as though the XGBoost was the best model at classifying which four 
